defmodule GPotion do
  ...
  defp gptype_server_(map) do
    receive do
      {:add_type, fun, types} ->
        map = Map.put(map, fun, types)
        gptype_server_(map)

      {:get_type, pid, fun} ->
        type = Map.get(map, fun)
  ...
  defmacro gpotion(header, do: body) do
    ...
    accessfunc =
      GPotion.CudaBackend.gen_kernel_call(fname, length(types_para), Enum.reverse(types_para))

    # ---------------- Isso estava causando problemas por redeclarar funções -----------------
    ...
    IO.puts("Creating kernel #{fname} in c_src/#{module_name}.cu")
    file = File.open!("c_src/#{module_name}.cu", [:write])
    IO.write(file, "#include \"erl_nif.h\"\n\n" <> k <> "\n\n" <> accessfunc)
    File.close(file)
    ...
    para =
      if is_list(List.last(para)) do
        List.delete_at(para, length(para) - 1)
      else
        para
      end
    ...
    # ------------- Esse é o código original para compilar usando o nvcc ---------------
    ...
    {result, errcode} =
      System.cmd(
        "hipify-clang",
        ["c_src/#{module_name}.cu", "-o", "c_src/#{module_name}.hip"],
        stderr_to_stdout: true
      )

    if errcode == 1 do
      raise "Error during hipify of .cu file generated by GPotion:\n#{result}"
    end
    ...
    {result, errcode} =
      System.cmd(
        ...
        [
          ...
          "-o",
          "priv/#{module_name}.so",
          "c_src/#{module_name}.hip"
        ],
        stderr_to_stdout: true
      )

    if errcode == 1 do
      raise "Error when compiling .hip file generated by GPotion:\n#{result}"
    end

    # Renaming the HIP file
    File.rename("c_src/#{module_name}.hip", "c_src/#{module_name}_gp.hip")
  ...
  def load(kernel) do
    case Macro.escape(kernel) do
      ...
      _ ->
        raise "GPotion.build: invalid kernel"
    end
  end
